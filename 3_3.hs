...
-- Моноид для объединения множеств
...
-- Моноид для пересечения множеств
instance Monoid (PSet a) where
    -- Универсальное множество нейтально, потому что с ним можно пересекать
    -- любое другое множество без изменения результата
    mempty = PSet (\ _ -> True)
    -- Элемент входит в пересечение, если удовлетворяет обоим предикатам
    mappend (PSet p1) (PSet p2) = PSet (\ el -> p1 el && p2 el)
